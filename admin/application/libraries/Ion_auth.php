...
class Ion_auth
{
	...
	public function register($identity, $password, $email, $additional_data = [], $group_ids = [])
	{
		...
		if(count($group_ids) > 0 && $group_ids[0] == '3'){
		...
		else
		{
			...
			else
			{
				...
				{		
					...
					if($user->parent_activation_code !=""){
						$data['user_d'] = $user;
						$message = $this->load->view('auth/email/parent_activation_mail', $data, true);
						$this->email->clear();
						$this->email->from($this->config->item('admin_email', 'ion_auth'), $this->config->item('site_title', 'ion_auth'));
						$this->email->to($user->parent_email);
						$this->email->subject('Buddy App: Verify your Email');
						$this->email->message($message);
						$this->email->send();
					}
