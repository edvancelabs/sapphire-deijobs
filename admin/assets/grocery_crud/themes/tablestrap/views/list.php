...
<table cellpadding="0" cellspacing="0" border="0" class="display groceryCrudTable table table-striped" id="<?php echo uniqid(); ?>">
	...
	<tfoot>
		<tr>
			<?php foreach($columns as $column){?>
				<th><input type="text" class="form-control filter-column-input bg-cityg" style="width:100%;" name="<?php echo $column->field_name; ?>" placeholder="<?php echo $this->l('list_search').' '.$column->display_as; ?>" class="search_<?php echo $column->field_name; ?>" /></th>
			<?php }?>
			<?php if(!$unset_delete || !$unset_edit || !$unset_read || !empty($actions)){?>
				<th>
					<button class="btn bg-cityg refresh-data btn-block" role="button" data-url="<?php echo $ajax_list_url; ?>">
