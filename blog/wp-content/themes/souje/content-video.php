...
$souje_featured_image_video = get_theme_mod( 'souje_featured_image_video', 'vid' );
...
<?php if ( !is_single() && souje_apply_columns() ) { echo '<div class="' . souje_apply_columns() . '">'; } ?>
	<article <?php post_class( 'clearfix' ); if ( !is_single() && !souje_check_style_z() && $souje_min_post_height ) { echo 'style="min-height: ' . esc_attr( $souje_min_post_height ) . 'px;"'; } ?>>
        ...
        <?php } else { ?>
            <?php
            $souje_videos = souje_featured_video();
            if ( $souje_videos ) {
                if ( is_single() ) {
                    echo souje_featured_video();
                } else {
                    if ( $souje_featured_image_video == 'vid' ) {
                        echo souje_featured_video();
                    } else if ( $souje_featured_image_video == 'fea' ) { ?>
                ...
                <div class="article-content-inner">
                    ...
                    <?php if ( is_single() ) { ?>
                        <div class="article-pure-content clearfix">
							<?php if ( $souje_videos ) {
								echo souje_remove_featured_video();
							} else {
								the_content();
							} ?>
                        </div>
