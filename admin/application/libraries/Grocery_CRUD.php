		...
		switch ($real_type) {
			...
			case 'upload_file':
				...
				// 	$file_url = base_url().$field_info->extras->upload_path."/$value";
					...
					$file_url = $field_info->extras->relative_dir."/$value";
			

					$file_url_anchor = '<a href="'.$file_url.'"';
					if($is_image)
					{
						$file_url_anchor .= ' class="image-thumbnail"><img src="'.$file_url.'" height="50px" style="width:50px;object-fit:contain;">';
					}
					else
					{
		...
		if(isset($this->upload_fields[$state_info->field_name]) )
		{
			if($this->callback_upload === null)
			{
				...
				// 	'upload_url'		=> base_url().$upload_info->upload_path.'/',
				    'upload_url'		=> $upload_info->relative_dir.'/',
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_success_message_at_list($field_info = null)
	{
		...
		{
			if(@$this->rfu_data['s_msg'] != ""){
				return $this->rfu_data['s_msg'];
			}
			return null;
		}
	}
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
	...
	protected function state_url($url = '', $is_list_page = false)
	{
		...
		if(@$this->rfu_data['show_filters'] && ($url == "ajax_list" || $url ==  "ajax_list_info" || $url ==  "export" )){
			$filter = "";
		...
		if(@$this->rfu_data['filter_list_url'] && $url == "" ){
			$state_url = $_SERVER['HTTP_REFERER'];
		}
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	public function set_rfu_data($rfu_data = null)
	{
		$this->rfu_data = $rfu_data;    
		return $this;
	}
	...
	public function set_field_upload($field_name, $upload_dir = '', $allowed_file_types = '')
	{
        ...
        $relative_dir = $this->CI->config->item('upload_base_path').'/'.$upload_dir;
        $upload_dir = $this->CI->config->item('upload_dir_path').$upload_dir;
