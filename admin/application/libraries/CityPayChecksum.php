...
class CityPayChecksum{
	...
	static private function verifySignatureByString($params, $key, $checksum){
		$cpay_hash = self::decrypt($checksum, $key);
		$salt = substr($cpay_hash, -4);
		return $cpay_hash == self::calculateHash($params, $salt) ? true : false;
	}
	...
	static public function createMRefrenceID($mId, $refId) {
		return $mId."_".$refId;
	}

	static public function getMRefrenceID($mRefId) {
		$a = explode("_", $mRefId,2);
		return $a[1];
	}

	static public function generateSignature($params, $key) {
		return hash('sha512', $params.$key);
	}

	static public function verifySignature($params, $key, $checksum){
		$hash = hash('sha512', $params.$key);
