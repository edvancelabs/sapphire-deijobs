...
class Auth extends CI_Controller
{
	...
	public function login()
	{
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
				...
				if($this->ion_auth->in_group('admin')){					
				...
				}else if($this->ion_auth->in_group('merchant')){
					...
					if($mid){
						$this->session->set_userdata(array('mid'=>$mid));
					}					
					redirect('merchant', 'refresh');
            
                }else{
					$this->db->select('merchant_id');
                	$this->db->where('staff_id', $this->ion_auth->get_user_id());
                	$r = $this->db->get('merchant_staff')->row();
                	if(@$r->merchant_id){
                    	$mid = $r->merchant_id;
                    	$this->session->set_userdata(array('mid'=>$mid));
                	}
 ...
 //            		}			
	...
	// public function parent_verify($id, $code){	

	// 	if ($code !== ""){
	// 		$this->db->where("parent_activation_code",$code);
	// 		$user = $this->db->get('users')->row();

	// 		if($user->id == $id){
	// 			//activate
	// 			$this->db->where('id',$id);
	// 			$this->db->update('users',array('parent_activation_code'=>null, 'parent_active'=>1));
	// 			echo "<h1>Email Verification Successful</h1>";
	// 		}else{
